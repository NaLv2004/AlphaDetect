...
def test_dce_T4() -> None:
    """T4: cpp ``behavioral_reduce_bp`` produces structurally identical
    reduced programs to the Python reference, on a fixed bank of frames.

    The Python reference outputs are pre-generated by
    ``pushgp.tests.regen_dce_golden`` and stored at
    ``pushgp/tests/data/dce_golden.pkl``.  This keeps CI fast (cpp only)
    while still proving end-to-end correctness against a golden trace.
    """
    import pickle
    from pathlib import Path as _Path
    from pushgp.dce import behavioral_reduce_bp, _try_import_pushgp_cpp_dce
    from pushgp.serialize import dict_to_program, program_to_dict

    if _try_import_pushgp_cpp_dce() is None:
        raise AssertionError(
            "pushgp_cpp_dce not importable; build cpp_dce/ before running T4"
        )

    golden_path = _Path(__file__).parent / "data" / "dce_golden.pkl"
    if not golden_path.exists():
        raise AssertionError(
            f"golden fixture missing: {golden_path}\n"
            "Regenerate with: python -m pushgp.tests.regen_dce_golden"
        )
    with golden_path.open("rb") as fh:
        blob = pickle.load(fh)
    t4 = blob["t4"]

    from ldpc_5g import build_parity
    par = build_parity(**t4["par_args"])
    rx_llrs = [np.asarray(a, dtype=np.float64) for a in t4["rx_llrs"]]
    max_iter   = int(t4["max_iter"])
    max_passes = int(t4["max_passes"])
    mde        = int(t4["max_decode_evals"])
    decimals   = int(t4["decimals"])
    mde_arg = (None if mde < 0 else mde)

    print(f"\n--- T4: cpp DCE vs Python golden  (n_pairs={len(t4['entries'])}) ---",
          flush=True)

    failures: List[str] = []
    for i, e in enumerate(t4["entries"]):
        v       = dict_to_program(e["v_orig"])
        c       = dict_to_program(e["c_orig"])
        k       = np.asarray(e["log_constants"], dtype=np.float64)
        v_gold  = e["v_red_py"]   # list-of-dict form
        c_gold  = e["c_red_py"]
        v_red_cpp = behavioral_reduce_bp(
            v, "v2c", peer_prog=c, log_constants=k, par=par,
            rx_llrs=rx_llrs, max_iter=max_iter, max_passes=max_passes,
            max_decode_evals=mde_arg, decimals=decimals, use_cpp=True,
        )
        c_red_cpp = behavioral_reduce_bp(
            c, "c2v", peer_prog=v, log_constants=k, par=par,
            rx_llrs=rx_llrs, max_iter=max_iter, max_passes=max_passes,
            max_decode_evals=mde_arg, decimals=decimals, use_cpp=True,
        )
        v_cpp_d = program_to_dict(v_red_cpp)
        c_cpp_d = program_to_dict(c_red_cpp)
        v_ok = (v_cpp_d == v_gold)
        c_ok = (c_cpp_d == c_gold)
        tag = "OK " if (v_ok and c_ok) else "FAIL"
        print(f"  [{i+1:2d}/{len(t4['entries'])}] {tag} "
              f"v cpp={len(v_cpp_d):2d} gold={len(v_gold):2d}  "
              f"c cpp={len(c_cpp_d):2d} gold={len(c_gold):2d}",
              flush=True)
        if not v_ok:
            failures.append(
                f"pair {i} V differs: cpp_len={len(v_cpp_d)} "
                f"gold_len={len(v_gold)}"
            )
        if not c_ok:
            failures.append(
                f"pair {i} C differs: cpp_len={len(c_cpp_d)} "
                f"gold_len={len(c_gold)}"
            )
    if failures:
        for f in failures[:20]:
            print("  FAIL:", f, flush=True)
        raise AssertionError(
            f"T4: {len(failures)} cpp/golden mismatches "
            f"(of {2*len(t4['entries'])} programs)"
        )
    print("  T4 PASSED", flush=True)
