"""Quick 5-gen evolution test with GNN pattern matcher."""
import sys, os
...
def _cb(gen, best_f, pop):
    ...
    grafted = sum(1 for g in pop if "grafted" in g.tags)
    print(f"  Gen {gen}: score={score:.6f}, unique={len(algos)}, grafted={grafted}")
    stats = gnn_matcher.get_stats()
    print(f"    GNN: proposals={stats['total_proposals']}, rewards={stats['total_rewards']:.4f}")
...
print("GNN stats:", gnn_matcher.get_stats())
